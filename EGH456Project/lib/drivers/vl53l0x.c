...
#define REG_GPIO_HV_MUX_ACTIVE_HIGH (0x84) // Keep even though we arent using GPIO intterupts??
...
static bool wait_for_boot()
{
    ...
    while ((xTaskGetTickCount() - start) < pdMS_TO_TICKS(100)) {
        if (i2c_read_addr8_data8(0xC0, &id) && id == VL53L0X_EXPECTED_DEVICE_ID) {
            UARTprintf("Device booted, ID = 0x%02X\n", id);
            return true;
        }
        vTaskDelay(pdMS_TO_TICKS(5));
    }
    UARTprintf("Device did not boot in time. Got ID = 0x%02X\n", id);
    return false;
}
...
static bool device_is_booted()
{
    ...
    if (!i2c_read_addr8_data8(0xC0, &boot_id)) {
        UARTprintf("Failed to read boot ID from 0xC0\n");
        return false;
    }

    UARTprintf("Device booted, ID = 0x%02X\n", boot_id);

    if (boot_id != VL53L0X_EXPECTED_DEVICE_ID) {
        UARTprintf("Device ID mismatch: expected 0x%02X, got 0x%02X\n",
                   VL53L0X_EXPECTED_DEVICE_ID, boot_id);
        return false;
    }
...
static bool init_address(vl53l0x_idx_t idx)
{
    ...
    if(!wait_for_boot()) {
            UARTprintf("VL53L0X: Device %d not booted after wait\n", idx);
            return false;
        }

    if (!device_is_booted()) {
        UARTprintf("VL53L0X: Device %d not booted\n", idx);
        return false;
    }

    if (!configure_address(vl53l0x_infos[idx].addr)) {
        UARTprintf("VL53L0X: Failed to configure address for idx %d\n", idx);
        return false;
    }

    UARTprintf("VL53L0X: Address configured for idx %d\n", idx);
    return true;
}
...
{
 for (int i = 0; i < sizeof(vl53l0x_infos) / sizeof(vl53l0x_infos[0]); i++) {
        UARTprintf("Setting slave addr to 0x%02X for device %d\n", VL53L0X_DEFAULT_ADDRESS, i);
        UARTprintf("Configuring address to 0x%02X\n", vl53l0x_infos[i].addr);

        i2c_set_slave_address(VL53L0X_DEFAULT_ADDRESS);
        vTaskDelay(pdMS_TO_TICKS(1));
        UARTprintf("Checking to see if device %d is booted\n", i);
        if (!device_is_booted()) {
            UARTprintf("VL53L0X: Device %d not booted\n", i);
            return false;
        }
        if (!configure_address(vl53l0x_infos[i].addr)) {
            UARTprintf("VL53L0X: Failed to configure address %d\n", i);
            return false;
        }
        UARTprintf("VL53L0X: Device %d configured\n", i);
    }
    UARTprintf("VL53L0X: All devices booted\n");
    return true;
}
...
bool vl53l0x_init()
{
    if (!init_address(VL53L0X_IDX_FIRST)) {
        UARTprintf("VL53L0X: Failed to init single address\n");
        return false;
    }
    if (!init_config(VL53L0X_IDX_FIRST)) {
        UARTprintf("VL53L0X: Failed to init config\n");
        return false;
    }
#ifdef VL53L0X_SECOND
