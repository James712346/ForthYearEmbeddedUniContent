...
main(void)
{
    ...
    Delay(2);
 //
    // Indicate that the GPIO B interrupt priority test is beginning.
    //
    g_ui32IntMode = GPIOB_PRIORITY;

    //
    // Set the interrupt priorities so that B > C > A.
    //
    IntPrioritySet(INT_GPIOA, 0x80);
    IntPrioritySet(INT_GPIOB, 0x00);
    ...
    if(ui8Error)
    {
        ...
        if (ui8Error & 16){
            UARTprintf("GPIO B Priority Fail!\n");
        }
    }
    else
    {
