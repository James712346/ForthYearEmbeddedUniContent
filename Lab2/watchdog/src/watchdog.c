...
SW1ButtonPressed(void)
{
    ...
    WatchdogReloadSet(WATCHDOG0_BASE, 5*g_ui32SysClock);
    return(0);
}
...
main(void)
{
    ...
    if (SysCtlResetCauseGet() & SYSCTL_CAUSE_WDOG0){
        UARTprintf("Restarting from WatchDog Reset\n");
    } else {
        UARTprintf("Starting WatchDog Program\n");
    }
    ...
    while(1)
    {
        ...
        if(ui8Buttons & USR_SW1)
        {
            ...
            UARTprintf("Prevented the watchdog from resetting\n");
        }
    }
}
